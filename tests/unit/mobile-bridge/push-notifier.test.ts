 ...
 *
 * Covered: the three trigger mappings (permission-needed vs
 * agent-question, thinking->idle turn-complete, unintentional-exit
 * session-failed), presence suppression for established devices, the
 * 30s per (device, session, category) cooldown, the 2s permission
 * debounce with its cleared-meanwhile skip, the envelope-only privacy
 * property (no plaintext field value anywhere in the POST body), and
 * the DeviceNotRegistered registration drop.
 */
...
class FakeSessionManager extends EventEmitter {
  ...
  getEventsForSession = vi.fn((): Array<{ ts: number; type: string; tool?: string; toolId?: string }> => [
    { ts: 1, type: 'tool_start', tool: 'Bash', toolId: 'tool-1' },
  ]);
}
...
describe('PushNotifier', () => {
  ...
  it('a transition into permission notifies permission-needed after the 2s debounce', async () => {
    buildNotifier();
    ...
    expect(sealedCategories()).toEqual(['permission-needed']);
  ...
  it('an awaited AskUserQuestion maps to agent-question', async () => {
    sessionManager.getEventsForSession.mockReturnValue([{ ts: 1, type: 'tool_start', tool: 'AskUserQuestion', toolId: 'tool-1' }]);
    buildNotifier();
    sessionManager.emit('activity', 'sess-1', 'permission', { kind: 'permission' });
    await vi.advanceTimersByTimeAsync(2000);
    expect(sealedCategories()).toEqual(['agent-question']);
    expect(postedBodies()[0].channelId).toBe('needs-attention');
    expect(postedBodies()[0].body).toBe('Agent needs your attention');
  });
