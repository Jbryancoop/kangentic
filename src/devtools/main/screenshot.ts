...
export async function captureScreenshotWithBudget(
  ...
  const [layoutResult, firstAttempt] = await Promise.all([
    getLayoutMetrics(window),
    captureOnce(window, { ...options, format, quality }),
  ]);
  ...
  while (attempt.byteLength > effectiveBudget) {
    ...
    const next = await captureOnce(window, nextOptions);
    if (!next) break;
    attempt = next;
  }
...
async function captureOnce(
  ...
  const base64 = await captureScreenshot(window, options);
...
export async function captureElementClip(
  ...
  return captureScreenshotWithBudget(window, {
    ...options,
    fullPage: false,
