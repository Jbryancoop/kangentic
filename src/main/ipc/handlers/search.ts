...
export function registerSearchHandlers(context: IpcContext): void {
  ipcMain.handle(IPC.SEARCH_EVERYTHING, async (_event, request: SearchRequest): Promise<SearchHit[]> => {
    ...
    // Projects: only meaningful when scope is 'all' (otherwise the user is
    // already in their current project and doesn't need a "switch" hit).
    if (request.scope === 'all') {
      pushProjectHits(allProjects, needleLower, budget, hits);
    }
    ...
    for (const project of projects) {
      pushTaskHits(project, needleLower, budget, hits);
      pushBacklogHits(project, needleLower, budget, hits);
    }
    ...
    await Promise.all(projects.map(async (project) => {
      try {
        await pushSessionEventHits(project, needleLower, budget, hits);
      } catch (err) {
